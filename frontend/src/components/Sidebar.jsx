...
export default function Sidebar({ layers, toggleLayer }) {
  ...
  const toggleAll = () => {
    ...
    LAYER_META.forEach((l) => {
      if (allActive !== !layers[l.id]) toggleLayer(l.id);
    });
  };
        ...
        {filtered.map((l) => {
          ...
          return (
            <div key={l.id} onClick={() => toggleLayer(l.id)}
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 10px', cursor: 'pointer', userSelect: 'none', transition: 'background 0.1s' }}
              ...
              <span style={{ fontSize: 11, fontWeight: 500, color: active ? 'var(--text)' : 'var(--text3)' }}>{l.name}</span>
            </div>
          );
        })}
      </div>
